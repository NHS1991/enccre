...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xpath-default-namespace="http://www.tei-c.org/ns/1.0">
    ...
    <xsl:template match="div" mode="page">
        <div class="page">
            <div class="content">
                <div id="doc_transform">
                   <xsl:apply-templates select="div|seg" mode="article"/>
                </div>
                <div id="doc_original">
                    <xsl:element name="img">
                        <xsl:variable name="volume">
                            <xsl:value-of select="substring-after(substring-before(@xml:id,'-'),'v')"/>
                        </xsl:variable>
                        <xsl:attribute name="src">
                            <xsl:choose>
                                <xsl:when test="$volume='2'">
                                    <xsl:value-of select="concat('../../../../Mazarine_1/texte/volume_',$volume,'_min/',@facs)"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of select="concat('../../../../Mazarine_1/texte/volume_',$volume,'/',@facs)"/>
                                </xsl:otherwise>
                            </xsl:choose>
    ...
    <xsl:template match="seg" mode="article">
        <span>
            <xsl:variable name="style">
                <xsl:if test="@style">
                    <xsl:value-of select="@style"/>
    ...
    <xsl:template match="ref">
        <a>
          <xsl:attribute name="href">
              <xsl:choose>
                  <xsl:when test="@target='#inconnu'">
                      <xsl:text>#article_inconnu</xsl:text>
                  </xsl:when>
                  <xsl:otherwise>
